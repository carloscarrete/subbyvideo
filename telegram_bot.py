...
def download_yt_video(message):
    ...
    if tg_video_option=='sub' or tg_video_option=='original':
        try:
            ...
            title_video2 = download_video(tg_video_yt, '.')
            title_video = title_video2.strip()
            ...
            if tg_video_option == 'sub':
                ...
                #SEND .srt file
                bot.reply_to(message, 'Tu video aun se esta procesando, pero tus subtítulos ya están listos, puedes descargarlos a continuación')
                shutil.copy(sub, '/var/www/html/videos/'+title_video + '.srt')
                bot.reply_to(message, SERVER + '/videos/'+urllib.parse.quote(title_video) +'.srt')
...
def handle_callback_query(call):
    ...
    if call.data == 'option1':
        ...
        if tg_video:
            bot.send_message(call.message.chat.id, 'Ahora bien, por favor envie un video para proceder con los subtítulos')
        download_yt_video(call.message)
        print('ORIGINAL')
